...
<div class="controls-content">
		...
		if($item['CODE']=='TYPE_APARTMENT'){$show_script[]='TYPE_APARTMENT';}?>
			<div class="paramete-box">
				...
				<div class="housing-line">
					...
					<label><input type="checkbox" name="<?=$item['CODE']?>[]" value="<?=$key?>" <?
			if(count($_REQUEST)<=0||in_array($key,$_REQUEST[$item['CODE']])){?>checked<?}?>><?=$val['VALUE']?></label>
					<?}?>
	...
	<?}elseif($item['CODE']=='ORENDA_FOR_MANS'){$show_script[]='ORENDA_FOR_MANS';?>
		...
		<?}?>
	<?}?>
<input type="submit" value="подбор по фильтру" class="podbor_link" style="border: 0;">
	</form>
...
<?if ($_REQUEST['PLOSH']){$min_ploh_val=min ($_REQUEST['PLOSH']);$max_ploh_val=max ($_REQUEST['PLOSH']);?>
$('input.minPrice[name="PLOSH[]"]').val(<?=$min_ploh_val?>).change();
$('input.maxPrice[name="PLOSH[]"]').val(<?=$max_ploh_val?>).change();
set_ofises();
<?}?>
