...
<?// получение информации о корпуcе и этаже на которых располагается офис
$ob_korpus=CIBlockElement::GetByID($arResult['PROPERTIES']['KORPUS']['VALUE']);
$arResult['KORPUS'] = $ob_korpus->GetNext();
$ob_plan=CIBlockElement::GetByID($arResult['PROPERTIES']['PLAN']['VALUE']);
$arResult['ARR_FLOOR'] = $ob_plan->GetNext();

$arResult['COUNT_OF_SLIDES'] =count($arResult['PROPERTIES']["IMG_GALERY"]['VALUE']);

?>

<? //получение координат для отрисовки SVG  карты этажа
$arResult['AR_OFFICE']=array();
$arFilter=Array('IBLOCK_ID'=>6,'PROPERTY_PLAN'=>$arResult['ARR_FLOOR']['ID'],'ACTIVE'=>'Y');
...
while($ar = $ob_ofise->GetNext()) {
    ...
    $arResult['COORDINATES_SVG'][$ar['ID']]=$col_value;

}
$ob_file_plan=CFile::GetByID($arResult['ARR_FLOOR']['PREVIEW_PICTURE']);
//получаем информацию о файле схемы этажа , нам понадобится ширина и высота
$arResult['FILE_PLAN'] = $ob_file_plan->GetNext();


//кэшируем пользовательские свойства для передачи в эпилог
