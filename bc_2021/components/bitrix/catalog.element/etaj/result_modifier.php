...
$ob_korpus=CIBlockElement::GetList(Array("SORT"=>"ASC"),Array('ACTIVE'=>'Y','IBLOCK_ID'=>6),false,false,
Array('ID','CODE','NAME','PREVIEW_PICTURE','PROPERTY_PLOSH','PROPERTY_ORENDA_FOR_MANS','PROPERTY_ORENDA_M_KV_GOD','PROPERTY_TYPE_APARTMENT','PROPERTY_PLAN.PROPERTY_NAME','PROPERTY_KORPUS.NAME'));
while($ar_fields = $ob_korpus->GetNext())
{
	$arResult['OFISES'][$ar_fields['ID']]=$ar_fields;
}
?>

<?

// Для переключения между этажами кнопками + и - получим ссылки на этажи
...
while($ob = $ob_el->GetNext()){
    if($ob['SORT']>$arResult['SORT']){$arResult['ETAHE']['+']=$ob;}else{$arResult['ETAHE']['-']=$ob;}
}
$ob_el=CIBlockElement::GetByID($arResult['PROPERTIES']['KORPUS']['VALUE']);
$arResult['AR_RES'] = $ob_el->GetNext();
?>


<? // получение координат для интерактивной схемы этажа
...
while($ar = $ob_ofise->GetNext()) {
    ...
    $arResult['AR_COORDINATES'][$ar['ID']]=$col_value;
